...
# Во ВКонтакте нет колбэков (callback query), как в Telegram
# поэтому мы не можем передавать данные через колбэки и используем глобальные словари
chat_histories = {}
llm_messages = {}

# message_handler - функция-обёртка для передачи данных в handle_message
# В bot_tg.py (Telegram) handle_message подключается напрямую к фреймворку.
# Здесь же нужна обёртка, чтобы передать глобальные словари в функцию обработки.
@labeler.message()
